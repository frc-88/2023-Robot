...
public class Handoff extends SequentialCommandGroup {
  ...
  public Handoff(Intake intake, Arm arm, Grabber grabber, boolean coneMode, boolean runShort) {
    ...
    addCommands(
      ...
      arm.sendArmToStateAndEnd(coneMode ? ArmStates.getConeFromIntake2 : ArmStates.getCubeFromIntake2).deadlineWith(grab.get(), intake.handoffFactory()).withTimeout(0.2),
      arm.sendArmToState(ArmStates.stow).alongWith(grab.get(), intake.handoffFactory()).until(() -> arm.isAtTarget(ArmStates.stow, runShort ? 30 : 2))
    );
  }
}
